...
static jmethodID s_AndroidHostInterface_method_onEmulationStarted;
static jmethodID s_AndroidHostInterface_method_onEmulationStopped;
static jmethodID s_AndroidHostInterface_method_setVibration;
                                                ...
                                                std::shared_ptr<SystemBootParameters> boot_params, bool resume_state)
{
  ...
  m_emulation_thread = std::thread(&AndroidHostInterface::EmulationThreadEntryPoint, this,
...
void AndroidHostInterface::EmulationThreadEntryPoint(ANativeWindow* initial_surface,
                                                     std::shared_ptr<SystemBootParameters> boot_params, bool resume_state)
{
  JNIEnv* thread_env;
  ...
  if (!boot_result)
  {
    ...
    DestroyImGuiContext();
    thread_env->CallVoidMethod(m_java_object, s_AndroidHostInterface_method_onEmulationStopped);
    s_jvm->DetachCurrentThread();
    return;
  }

  // System is ready to go.
  thread_env->CallVoidMethod(m_java_object, s_AndroidHostInterface_method_onEmulationStarted);
  EmulationThreadLoop();
  thread_env->CallVoidMethod(m_java_object, s_AndroidHostInterface_method_onEmulationStopped);
...
void AndroidHostInterface::SetVibration(bool enabled)
{
  ...
  JNIEnv* env = AndroidHelpers::GetJNIEnv();
  env->CallVoidMethod(m_java_object, s_AndroidHostInterface_method_setVibration,
...
extern "C" jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
  ...
  if ((string_class = env->FindClass("java/lang/String")) == nullptr ||
      ...
      (host_interface_class = env->FindClass("com/androidemu/mame/AndroidHostInterface")) == nullptr ||
      (s_AndroidHostInterface_class = static_cast<jclass>(env->NewGlobalRef(host_interface_class))) == nullptr ||
      (patch_code_class = env->FindClass("com/androidemu/mame/PatchCode")) == nullptr ||
  ...
  if ((s_AndroidHostInterface_constructor =
      ...
      (s_AndroidHostInterface_method_onEmulationStarted = env->GetMethodID(
		 s_AndroidHostInterface_class, "onEmulationStarted", "()V")) == nullptr ||
      (s_AndroidHostInterface_method_onEmulationStopped = env->GetMethodID(
		 s_AndroidHostInterface_class, "onEmulationStopped", "()V")) == nullptr ||
      (s_AndroidHostInterface_method_setVibration = env->GetMethodID(
		s_AndroidHostInterface_class, "setVibration", "(Z)V")) ==
        nullptr ||
...
#define DEFINE_JNI_METHOD(return_type, name)                                                                           \
  extern "C" JNIEXPORT return_type JNICALL Java_com_androidemu_mame_##name(JNIEnv* env)

#define DEFINE_JNI_ARGS_METHOD(return_type, name, ...)                                                                 \
  extern "C" JNIEXPORT return_type JNICALL Java_com_androidemu_mame_##name(JNIEnv* env, __VA_ARGS__)
...
{
  return env->NewStringUTF(SmallString::FromFormat("%s (%s)\nBuilt %s %s", g_scm_tag_str,
...
DEFINE_JNI_ARGS_METHOD(jboolean, AndroidHostInterface_startEmulationThread, jobject obj,
                       jobject surface, jstring filename, jboolean resume_state, jstring state_filename)
{
  ...
  return AndroidHelpers::GetNativeClass(env, obj)->StartEmulationThread(native_surface,
...
{
  /*jclass entry_class = env->FindClass("com/androidemu/mame/GameListEntry");
